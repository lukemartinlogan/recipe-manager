...
class MealPlan:
    ...
    def load_usda_food_composition():
        food_comp_path = f'{FOOD_ROOT}/datasets/usda/food_comp.parquet'
        food_comp = pd.read_parquet(food_comp_path)
        food_comp = food_comp.fillna(0)
        return food_comp

    @staticmethod
    def load_usda_food_names():
    ...
    @staticmethod
    def load_usda_dv():
        with open(f'{FOOD_ROOT}/datasets/usda/food_dv.yaml') as f:
            dv = yaml.safe_load(f)
        return dv
