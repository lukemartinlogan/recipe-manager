class PackedYogurt:
    def __init__(self, plan):
        ...
        plan.ingest('Peanuts, raw', 30)
        plan.ingest('Protein Powder, Vanilla, Body Fortress', 20)
...
class CookedSpinach:
    def __init__(self, plan):
        ...
        """
        plan.ingest('Spinach, mature', 60)
        plan.ingest('Oil, olive, extra virgin', 10)
...
class PorkSandwich:
    def __init__(self, plan):
        plan.ingest('Avocado, raw', 50)
        plan.ingest('Pork, chop, lean only eaten', 150)
        plan.ingest('Eggs, Grade A, Large, egg whole', 100)
        plan.ingest('WHOLE WHEAT BREAD', 100)
...
class SalmonDinner:
    def __init__(self, plan, salmon='fresh'):
        ...
        plan.ingest('Oil, olive, extra virgin', 15)
        plan.ingest('PINK HIMALAYAN SALT, PINK', .400)  # 400 mg
        plan.ingest('Rice, brown, long grain, unenriched, raw', 100)

        # plan.ingest('SIGNATURE MASHED POTATOES', 125)
        plan.ingest('Sweet Peas, Canned, Del Monte', 70)
        plan.ingest('Beets, Canned, Sliced, Del Monte', 234)
        CookedSpinach(plan)
        CookedAsparagus(plan, 200)
...
class GarlicShrimp:
    def __init__(self, plan):
        plan.ingest('Crustaceans, shrimp, farm raised, raw', 150)
        plan.ingest('UNSALTED PURE IRISH BUTTER, UNSALTED', 14)
